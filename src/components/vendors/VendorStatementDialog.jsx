...
import { getVendorStatement, PR_STATUS_COLORS } from '../payments/paymentData';
...
function VendorStatementDialog({ open, onClose, vendor }) {
  ...
  useEffect(() => {
    if (!open || !vendor) { setStmt(null); return; }

    const vendorId = vendor._id || vendor.id;
    setLoading(true);

    Promise.all([
      paymentRequestsAPI.getAll({ vendor: vendorId }).catch(() => ({ paymentRequests: [] })),
      tdsAPI.getAll().catch(() => ({ tdsRecords: [] })),
    ]).then(([prRes, tdsRes]) => {
      const prs = prRes.paymentRequests || [];
      const tds = tdsRes.tdsRecords || [];
      setStmt(getVendorStatement(vendorId, prs, tds));
      ...
      <DialogContent sx={{ pt: 2.5 }}>
        ...
        {loading ? (
          ...
          <>
            ...
            {/* 30K Threshold Warning */}
            {(stmt.totalGross + stmt.pendingAmount) > 30000 && (
              <Alert
                severity="warning"
                icon={<WarningIcon />}
                sx={{ mb: 2.5, borderRadius: 2, border: '1px solid #fde68a' }}
              >
                <Typography variant="body2" fontWeight={700}>
                  Cumulative payments to this PAN exceed ₹30,000
                </Typography>
                <Typography variant="caption" color="text.secondary">
                  Total (paid + pending): {fmtINR(stmt.totalGross + stmt.pendingAmount)} — Please verify TDS compliance.
                </Typography>
              </Alert>
            )}
                    ...
                    {stmt.requests.map((r) => {
                      ...
                      return (
                        <TableRow key={r.id} hover>
                          ...
                          <TableCell>
                            {r.status === 'Payment Done' && parseFloat(r.tdsAmount) > 0 ? (
                              r.tdsDeposited ? (
